...
def _trace(message: str) -> None:
    ...
    if not _trace_enabled() or _trace_counter >= _TRACE_LIMIT:
        return
    _trace_counter += 1
    print(f"[opd.trace.engine] {message}", flush=True)
...
class Engine:
    def __init__(self, config: EngineConfig, *, model=None):
        ...
        self._embedded_mode = config.embedded_mode
    ...
    def _determine_num_pages(self, old_free_memory: int, config: EngineConfig) -> int:
        ...
        if num_pages is None:
            ...
            used = total - free
            available_memory = int(config.memory_ratio * total) - used
            logger.info_rank0(
                f"KV budget: total={mem_GB(total)}, used={mem_GB(used)}, "
                f"ratio={config.memory_ratio}, "
    ...
    def forward_batch(self, batch: Batch, args: BatchSamplingArgs) -> ForwardOutput:
        ...
        if logits.shape[0] > 0:
            ...
            _trace(
                f"path={path} batch_size={batch.size} padded_size={batch.padded_size} "
                f"top_ids={top_ids.tolist()} top_vals={[round(float(v), 4) for v in top_vals.tolist()]}"
            )
        ...
        _trace(f"next_tokens_gpu_head={next_tokens_gpu[: min(8, next_tokens_gpu.numel())].tolist()}")
...
def _resolve_config(config: EngineConfig) -> EngineConfig:
    """Return a new config with auto-detected fields resolved."""
    ...
    attn = overrides.get("attention_backend", config.attention_backend)
    if "trtllm" in attn and config.page_size not in [16, 32, 64]:
