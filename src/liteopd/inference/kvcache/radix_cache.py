...
from liteopd.inference.utils import align_down
...
class RadixTreeNode:
    ...
    def get_match_len(self, input_ids: torch.Tensor) -> int:
        from liteopd.inference.kernel import fast_compare_key
...
class RadixPrefixCache(BasePrefixCache):
        ...
        if unlock:
            while not node.is_root():
                ...
                if node.ref_count == 0:
                    ...
                    if node.is_leaf():
                        self.evictable_leaf_size += node.length
                node = node.parent
        else:
            while not node.is_root():
                if node.ref_count == 0:
    ...
    def insert_prefix(self, input_ids: torch.Tensor, indices: torch.Tensor) -> InsertResult:
        ...
        if prefix_len != insert_len:  # NOTE: prefix_len < insert_len
            ...
            # parent (node) is no longer a leaf after this insert
            if node.is_leaf() and node.ref_count == 0:
                self.evictable_leaf_size -= node.length
            new_node.set_parent(node)
            self.evictable_size += new_node.length
            self.evictable_leaf_size += new_node.length  # new_node is a leaf with ref_count==0
            node = new_node
        return InsertResult(prefix_len, RadixCacheHandle(insert_len, node))

    def evict(self, size: int) -> torch.Tensor:
        if size == 0 or self.evictable_leaf_size == 0:
        ...
        while evicted_size < size:
            ...
            if parent.is_leaf() and parent.ref_count == 0:
                self.evictable_leaf_size += parent.length
    ...
    def size_info(self) -> SizeInfo:
        return SizeInfo(
            ...
            evictable_leaf_size=self.evictable_leaf_size,
        )
