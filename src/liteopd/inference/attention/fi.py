...
import torch
from liteopd.inference.core import Batch, get_global_ctx
from liteopd.inference.distributed import get_tp_info
from liteopd.inference.env import ENV
from liteopd.inference.utils import div_even, init_logger
...
class FIMetadata(BaseAttnMetadata):
    ...
    swa_wrapper:        BatchPrefillWithPagedKVCacheWrapper | BatchDecodeWithPagedKVCacheWrapper | None = None
...
class FlashInferBackend(BaseAttnBackend):
    def __init__(self, config: ModelConfig) -> None:
        ...
        self._sliding_window_sizes = config.sliding_window_sizes
        self._sm_scale = config.attn_scale

        # Sliding window: create a second set of wrappers for SWA layers
        self._has_swa = any(w > 0 for w in self._sliding_window_sizes)
        self._swa_window_left = max((w for w in self._sliding_window_sizes if w > 0), default=-1)
        if self._has_swa:
            self.swa_float_workspace_buffer = torch.empty(
                128 * 1024 * 1024, dtype=torch.uint8, device=self.device
            )
            self.swa_prefill_wrapper = BatchPrefillWithPagedKVCacheWrapper(
                self.swa_float_workspace_buffer,
                kv_layout="NHD",
                backend="fa2",
            )
            self.swa_decode_wrapper = BatchDecodeWithPagedKVCacheWrapper(
                self.swa_float_workspace_buffer,
                use_tensor_cores=self.use_tensor_cores,
                kv_layout="NHD",
                backend="fa2",
            )
            self.swa_prefill_wrapper._int_workspace_buffer = self.int_workspace_buffer
            self.swa_decode_wrapper._int_workspace_buffer = self.int_workspace_buffer
        else:
            self.swa_prefill_wrapper = None
            self.swa_decode_wrapper = None
        ...
        self.swa_graph_wrappers: Dict[int, CUDAGraphBatchDecodeWithPagedKVCacheWrapper] = {}
    ...
    def _initialize_metadata_once(self, metadata: FIMetadata) -> None:
        ...
        def _plan_wrapper(wrapper):
            if isinstance(wrapper, BatchDecodeWithPagedKVCacheWrapper):
                wrapper.plan(
                    ...
                    non_blocking=True,
                    window_left=self._swa_window_left if wrapper is not metadata.wrapper else -1,
        ...
        _plan_wrapper(metadata.wrapper)
        if metadata.swa_wrapper is not None:
            _plan_wrapper(metadata.swa_wrapper)
        self.last_event.record()
    ...
    def forward(
        ...
        if self._sliding_window_sizes[layer_id] > 0 and metadata.swa_wrapper is not None:
            return metadata.swa_wrapper.run(q=q, paged_kv_cache=kv_cache)
    ...
    def prepare_metadata(self, batch: Batch) -> None:
        ...
        batch.attn_metadata = FIMetadata(
            ...
            swa_wrapper=(self.swa_decode_wrapper if batch.is_decode else self.swa_prefill_wrapper) if self._has_swa else None,
        )
    ...
    def prepare_for_capture(self, batch: Batch) -> None:
        ...
        swa_graph_wrapper = None
        if self._has_swa:
            self.swa_graph_wrappers[bs] = CUDAGraphBatchDecodeWithPagedKVCacheWrapper(
                self.swa_float_workspace_buffer,
                kv_layout="NHD",
                use_tensor_cores=self.use_tensor_cores,
                indptr_buffer=capture.cu_seqlens_k[: bs + 1],
            ...
            self.swa_graph_wrappers[bs]._backend = "fa2"
            self.swa_graph_wrappers[bs]._int_workspace_buffer = self.int_workspace_buffer
            swa_graph_wrapper = self.swa_graph_wrappers[bs]

        self.prepare_metadata(batch)
        metadata = batch.attn_metadata
        assert isinstance(metadata, FIMetadata)
        metadata.wrapper = self.graph_wrappers[bs]
        metadata.swa_wrapper = swa_graph_wrapper
        self._initialize_metadata_once(metadata)
    ...
    def prepare_for_replay(self, batch: Batch) -> None:
        ...
        metadata.wrapper = self.graph_wrappers[bs]
        metadata.swa_wrapper = self.swa_graph_wrappers.get(bs)
        self._initialize_metadata_once(metadata)
